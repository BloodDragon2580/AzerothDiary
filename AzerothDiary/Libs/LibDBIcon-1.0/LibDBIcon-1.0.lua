...
local function onLeaveCompartment(self, menu)
    ...
    local buttonName = menu.text
    local object = lib.objects[buttonName]
    if object and object.dataObject and object.dataObject.OnLeave then
...
function lib:RemoveButtonBorder(name)
    local button = lib:GetMinimapButton(name)
    if button and button.border then button.border:Hide() end
end
function lib:SetButtonBorder(name, borderTexture, size, framePoint, offsetX, offsetY)
    local button = lib:GetMinimapButton(name)
    if button and button.border then
        lib:ResetButtonBorder(name)
        if type(borderTexture) == "number" or type(borderTexture) == "string" then button.border:SetTexture(borderTexture) end
...
function lib:ResetButtonBorder(name)
    ...
    if button and button.border then
        ...
        if WOW_PROJECT_ID == WOW_PROJECT_MAINLINE then button.border:SetSize(50, 50) else button.border:SetSize(53, 53) end
    end
end
...
function lib:RemoveButtonBackground(name)
    local button = lib:GetMinimapButton(name)
    if button and button.background then button.background:Hide() end
end
function lib:SetButtonBackground(name, backgroundTexture, size, framePoint, offsetX, offsetY)
    ...
    if button and button.background then
        lib:ResetButtonBackground(name)
        if type(backgroundTexture) == "number" or type(backgroundTexture) == "string" then button.background:SetTexture(backgroundTexture) end
...
function lib:SetButtonIcon(name, iconTexture, size, framePoint, offsetX, offsetY)
    ...
    if button and button.icon then
        lib:ResetButtonIcon(name)
        if type(iconTexture) == "number" or type(iconTexture) == "string" then button.icon:SetTexture(iconTexture) end
